// ====================== OVERWORLD.JS — v3.6 ИСПРАВЛЕННЫЙ ======================
...
class OverworldScene extends Phaser.Scene {
    ...
    create() {
        ...
        for (let x = 0; x < this.gridSize; x++) {
            for (let y = 0; y < this.gridSize; y++) {
                const isGrass = Math.random() < 0.6;   // увеличил шанс травы
        ...
        console.log('%c🌍 Overworld v3.6 — готов к бою', 'color:#C084FC');
    }
    ...
    movePlayer(dir) {
        ...
        if (tile && tile.getData('grass') && Math.random() < 0.7) {   // высокий шанс
            console.log('%c🌿 Наступил на траву — запускаю бой!', 'color:#ff0; font-weight:bold');
    ...
    triggerZoneBattle() {
        ...
        const playerHero = currentParty[0] || myHeroes[0] || { ru: "Пикачу", types: ["Electric"], hp: 130, maxhp: 130, sprite: "https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/25.png" };

        console.log('%c⚔️ Запуск боя!', 'color:#f00; font-size:16px');
...
function createControls() {
    ...
    controls.innerHTML = `
        ...
        <button onclick="window.forceBattle()" style="grid-column:1/-1; margin-top:10px; padding:12px; background:#C084FC; color:#0a001f; border:none; border-radius:16px; font-weight:bold;">
            ⚔️ Force Battle (тест)
        </button>
    `;
...
window.forceBattle = function() { if (overworldSceneInstance) overworldSceneInstance.triggerZoneBattle(); };
